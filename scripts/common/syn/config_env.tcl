...
if {[file exists $build_env_db_name]} {
    set_global_assignment -name TEXT_FILE ${build_env_db_name}
    ...
    foreach line [split $v "\n"] {
        ...
        if {[llength $e] == 2} {
            ...
            if { [info exists ::env($key)] } {
                if {$val != $::env($key)} {
                    post_message -type info "Updating ::env($key) from \"$::env($key)\" to \"$val\""
                    set ::env($key) $val
                }
            } else {
...
if { ![info exists ::env(BUILD_ROOT_REL)] } {
    ...
    # Print the definition of BUILD_ROOT_REL once across all programs in
    # a flow.
    if { ![info exists ::env(BUILD_ROOT_REL_PRINTED)] } {
        post_message -type info "BUILD_ROOT_REL is $::env(BUILD_ROOT_REL)"
        set ::env(BUILD_ROOT_REL_PRINTED) 1
    }
}

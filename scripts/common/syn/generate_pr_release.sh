...
quartus_sh -t "${OFS_ROOTDIR}"/ofs-common/scripts/common/syn/emit_project_macros.tcl --project=${Q_PROJECT} --revision=${Q_PR_REVISION} --mode=sim --output="${TGT_BUILD}"/platform/sim/fim_project_macros.txt
...
# Create a link to the project directory with a standard name and location
# to make it easier for scripts.
ln -s -T "${QUARTUS_BUILD_ROOT}" "${TGT_BUILD}"/quartus_proj_dir
...
export Q_PR_PARTITION_NAME_2="${Q_PR_PARTITION_NAME_2}"
export Q_PR_PARTITION_NAME_3="${Q_PR_PARTITION_NAME_3}"
export Q_PR_PARTITION_NAME_4="${Q_PR_PARTITION_NAME_4}"
export PR_COMPILE=1
EOF

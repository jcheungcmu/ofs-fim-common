...
def path_rel_to_ofs_root(path):
    '''
    Convert a file path to one that is relative to OFS_ROOTDIR.
    If successful, return a string with ${OFS_ROOTDIR}/newpath.
    If unsuccessful, return an absolute path.
    '''

    root = os.environ.get('OFS_ROOTDIR')
    if not root:
        return os.path.abspath(path)

    rel_path = os.path.relpath(path, root)
    if rel_path[:2] == '..':
        # path is outside OFS_ROOTDIR
        return os.path.abspath(path)

    return os.path.join('${OFS_ROOTDIR}', rel_path)
...
def write_vcs_script(
    ...
    ip_flist = os.path.join(output_dir, filename+".f")
    # Generate a path to ip_flist, including $OFS_ROOTDIR, for use in
    # a shell script.
    ip_flist_path = path_rel_to_ofs_root(ip_flist)
    ...
    if not infile:
        ...
        # IP files
        write_vcs_ip(fout, has_template, "", ip_flist_path, False)
    else:
        ...
        for line in fin:
            ...
            # IP files
            skip_line = write_vcs_ip(fout, has_template, line, ip_flist_path, skip_line)
...
def write_vcs_ip(fout, has_template, line, ip_flist_path, cur_skip_line):
    '''
